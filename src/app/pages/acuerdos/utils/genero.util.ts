 ...
 */
import { COMPANY_KEYWORDS_REGEX, WHITE_SPACE_REGEX, ACCENTS_REGEX } from '../patterns/patrones-regex';
 ...
 */
export function determineGeneroPropietario(fullName: string): 'masculino' | 'femenino' | 'empresa' {
  if (!fullName) return 'masculino';

  const cleanName = fullName.trim().toUpperCase();

  if (COMPANY_KEYWORDS_REGEX.test(cleanName)) {
    return 'empresa';
  }
  const parts = cleanName.split(WHITE_SPACE_REGEX);
  ...
  const normalized = firstName.normalize('NFD').replace(ACCENTS_REGEX, '');
 ...
 */
export function determineGeneroProducto(description: string): 'masculino' | 'femenino' {
  ...
  const words = cleanText.split(WHITE_SPACE_REGEX).filter(w => w.length > 2); // Palabras de más de 2 caracteres
  ...
  // Quitar acentos
  const normalizedSingular = singularNoun.normalize('NFD').replace(ACCENTS_REGEX, '');
